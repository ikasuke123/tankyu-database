...
if menu == "データを見る":
    st.header("🔍 登録データ一覧")
    try:
        # 読み込みは公開リンクからCSVとして取得（これが最も安定します）
    ...
    with st.form("entry_form", clear_on_submit=True):
        ...
        # 【重要】if submitted は必ずフォームの「中（インデント内）」に書く
        if submitted:
            if author and title:
                try:
                    # 新しいデータ行を作成
                    ...
                    # 先ほどの「Service Account」エラーが出た場合の対策案内
                    if "Service Account" in str(e):
                        st.error("Googleのセキュリティ制限により、直接の書き込みがブロックされました。")
                        st.info("解決するには：Streamlitの『Secrets』設定にGoogleの鍵情報を登録する必要があります。")
                    else:
                        st.error(f"書き込み失敗: {e}")
